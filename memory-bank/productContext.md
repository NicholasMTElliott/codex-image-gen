...
## Functional intent
- Inputs: `--style` (visual treatment), `--subject` (what to depict), `--generate N` (default 1), `--select M` (default 1, ≤ N), `--name SLUG` (optional, output filename slug), `--out DIR` (optional, persistent output dir override), `--debug` (flag, default off).
