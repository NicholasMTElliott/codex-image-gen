...
- **CLI shape:** subcommand dispatch — first positional arg picks `generate` or `edit` (default `generate` if absent or flag-first, for backward compat with 0.2.x callers).
  ...
  - Common: `--generate`, `--select` optional with int validation; `--name` optional slug (`[A-Za-z0-9._-]+`); `--out` optional dir (relative or absolute); `--debug` optional flag (preserves tmp on success). `-h`/`--help` prints usage to stdout and exits 0 (POSIX convention — pipeable). Usage-due-to-error (missing required arg, invalid number, invalid slug, mutually-exclusive conflict, missing/empty/unreadable prompt file, unknown @-token, bad reference) prints to stderr and exits 2.
...
- **Output contract:** JSON on stdout via `emit()`. Always the same shape (with mode-conditional extras: `references[]` and `instruction.{raw,resolved}` in edit mode). Exit code 0 iff `ok`.
